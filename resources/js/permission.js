...
const whiteList = ['/login', '/auth-redirect', '/registration', '/sending-response', '/verify', '/password-reset-step-1', '/password-reset-step-2', 'password/reset', '/sendagain']; // no redirect whitelist
  ...
  if (hasToken) {
    ...
    if (to.path.indexOf('/verify') !== -1) {
      ...
      if (whiteList.includes(to.path) || whiteList.includes(to.name)) {
        ...
        if (to.path === '/sending-response' || to.path === '/password-reset-step-2') {
          if (to.params.permissions && to.params.permissions.includes('fetchRedirect')) {
            next();
          } else {
            // next(`/login?redirect=${to.path}`);
            next(`/login`);
